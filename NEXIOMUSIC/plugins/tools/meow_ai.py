...
client_ai = AsyncGroq(api_key=GROQ_API_KEY) if GROQ_API_KEY else None
...
async def ai_reply(chat_id, user_id, text):
    ...
    system_prompt = (
        ...
        "Reply short (1-2 lines), funny, emotional. "
...
async def meow_ai_handler(client: Client, message: Message):
    ...
    # group me sirf reply ya mention pe
    if message.chat.type != "private":
        if not message.reply_to_message:
            if not message.text.lower().startswith(("meow", "meowstric")):
                return

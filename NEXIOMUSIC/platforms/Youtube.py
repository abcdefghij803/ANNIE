...
class YouTubeAPI:
    ...
    async def download(
        ...
        if songvideo:
            ...
            direct = True
            downloaded_file = await download_song(link)
        return downloaded_file, direct            print(f"✅ Download completed: {filepath}")
            return str(filepath)
...
class YouTubeAPI:
    ...
    async def download(
        ...
        if songvideo:
            ...
            direct = True
            downloaded_file = await download_song(link)
        return downloaded_file, direct    formats = info.get('formats', [])
    if not formats:
        print("No formats found.")
        return None
        ...
        if video:
            if await is_on_off(1):
                ...
                if stdout:
                    ...
                    total_size_mb = file_size / (1024 * 1024)
                    if total_size_mb > 250:
                        print(f"File size {total_size_mb:.2f} MB exceeds the limit.")
                        return None, True
                    downloaded_file = await loop.run_in_executor(None, lambda: yt_dlp.YoutubeDL({
                        "format": "(bestvideo[height<=?720][width<=?1280][ext=mp4])+(bestaudio[ext=m4a])",
                        "outtmpl": "downloads/%(id)s.%(ext)s",
                        "quiet": True,
